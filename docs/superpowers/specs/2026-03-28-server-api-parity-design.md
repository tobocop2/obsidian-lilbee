...
| Settings visibility | Progressive disclosure | Common settings visible, advanced behind toggle |
| Settings fields | All writable fields editable | Only infra/security truly read-only |
...
| Auth | Read server.json (managed), localhost bypass (external) | Server uses bearer tokens for mutating endpoints |

## Auth: Bearer Token Handling

The lilbee server uses bearer token auth for all mutating endpoints (POST, PUT, DELETE). The token is generated at server startup and written to `{data_dir}/server.json` as `{"token": "xxx"}`.

### Read-only endpoints (no auth needed)
`/api/health`, `/api/status`, `/api/search`, `/api/config`, `/api/documents`, `/api/models`, `/api/models/catalog`, `/api/models/installed`

### Authenticated endpoints (all others)
`/api/ask`, `/api/chat`, `/api/sync`, `/api/add`, `/api/models/pull`, `/api/models/chat PUT`, `/api/models/vision PUT`, `/api/models/show POST`, `/api/models/delete`, `/api/documents/remove`, `/api/crawl`, `PATCH /api/config`, `PUT /api/models/embedding`

### Plugin behavior

**Managed mode**: Plugin reads token from `pluginDir/server-data/data/server.json` after server starts. Passes to `LilbeeClient` which includes `Authorization: Bearer <token>` on all mutating requests.

**External mode**: Plugin has no token. Requests go without auth. Server must skip auth for localhost connections (127.0.0.1) — small addition to the server-side change.

**Token refresh**: On 401 response, re-read `server.json` and retry once. Handles server restarts.

### `LilbeeClient` changes
- Constructor accepts optional `token` parameter
- `fetch()` helper includes `Authorization: Bearer <token>` header when token is set
- New `loadToken(dataDir)` method reads from `server.json`
- 401 handling: re-read token, retry once, throw on second failure

### Server change (in `feature/api-config-endpoint`)
Add localhost bypass to `_AuthMiddleware`:
```python
...
| `src/api.ts` | Modify | Remove OllamaClient, add 11 new methods, add auth token support |
| `src/main.ts` | Modify | Remove Ollama, add 3 commands, load auth token |
| `src/server-manager.ts` | Modify | Remove Ollama env and option |
| `src/settings.ts` | Modify | Restructure with progressive disclosure, add PATCH wiring |
...
2. API client — remove OllamaClient, add new endpoints + auth token support (`api.ts`)
3. Core — remove Ollama from main.ts, server-manager.ts, add token loading (`main.ts`)
