...
- The first-run wizard's managed-server step now shows three phases — Downloading, Starting, Ready — each with a dot that lights up as it happens, an indeterminate bar with a "first run can take up to a minute" note, and a green "lilbee server is running" check when the server answers. The step is properly gated: if the download or startup fails, the wizard surfaces the error and stays put with a retry instead of advancing to the model picker without a running server.
- Browse Catalog shows model compatibility with the connected server. Unsupported models (architectures the server can't run) are dimmed, carry an "Unsupported" badge, and their Download button is disabled with a tooltip naming the architecture; models the server can't classify get an "Unverified" badge but stay downloadable. Applies to both grid and list views.
