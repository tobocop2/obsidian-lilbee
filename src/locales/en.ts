...
export const MESSAGES = {
    ...
    DESC_AGENT_SECTION:
        "Point a coding agent at this vault. lilbee writes the agent's config so it can search your notes, and for some agents run on your local models.",
    LABEL_AGENT_DETECTED: "Detected agents",
    DESC_AGENT_DETECTED: "Agent CLIs found on this machine.",
    PILL_AGENT_DETECTED: "Installed",
    PILL_AGENT_MISSING: "Not found",
    BUTTON_AGENT_RESCAN: "Rescan",
    LABEL_AGENT_CHOICE: "Agent",
    DESC_AGENT_CHOICE: "The agent lilbee keeps wired to this vault.",
    AGENT_OPTION_NONE: "None",
    LABEL_AGENT_KEEP_FRESH: "Keep the agent connected",
    DESC_AGENT_KEEP_FRESH:
        "Rewrite the agent's config each time the server starts. The port and access token change every boot, so without this the agent loses the connection.",
    LABEL_AGENT_MODEL: "Agent model",
    DESC_AGENT_MODEL: "The agent runs on lilbee's chat model. Changing it here changes it everywhere.",
    BUTTON_AGENT_CHANGE_MODEL: "Change model",
    AGENT_CONTEXT_BADGE: (tokens: number): string => `${tokens.toLocaleString()} token context`,
    AGENT_CONTEXT_UNKNOWN: "Context window unknown until the chat model loads.",
    AGENT_CONTEXT_WARNING: "Agents need strong tool calling and a 20k+ token context window.",
    LABEL_AGENT_CLAUDIAN: "Claudian",
    DESC_AGENT_CLAUDIAN_CONFIGURED: "lilbee turned on Claudian's opencode provider and made this model visible.",
    PILL_AGENT_CLAUDIAN_CONFIGURED: "Configured",
    BUTTON_OPEN_CLAUDIAN: "Open Claudian",
    DESC_AGENT_CLAUDIAN_MISSING:
        "Claudian runs opencode inside Obsidian. Install it from the community plugin store to chat with the agent without leaving your vault.",
    DESC_AGENT_CLAUDIAN_SKIPPED: "lilbee could not read Claudian's settings. Finish setup in Claudian's settings.",
    AGENT_STATUS_WRITTEN: (relative: string, port: string): string =>
        port ? `Config written ${relative} · port ${port}` : `Config written ${relative}`,
    AGENT_STATUS_PENDING: "Config is written when the server starts.",
    AGENT_DETECT_UNAVAILABLE: "lilbee could not check for agents. Make sure the server is running, then rescan.",
    AGENT_STATUS_GLOBAL: "hermes reads one global config, so lilbee does not write a file. Copy the block below.",
    LABEL_AGENT_COPY_CONFIG: "Config to copy",
    BUTTON_AGENT_COPY: "Copy",
    NOTICE_AGENT_COPIED: "Config copied.",
    AGENT_EMPTY_TITLE: "No coding agents found",
    AGENT_EMPTY_BODY: "Install one of these, then rescan. lilbee does not install or update agents itself.",
    AGENT_EMPTY_BREW: "brew install opencode",
    NOTICE_AGENT_CONNECTED: (agent: string, path: string): string => `${agent} is connected. Wrote ${path}.`,
    NOTICE_AGENT_CONNECTED_GLOBAL: (agent: string): string =>
        `${agent} config is ready to copy from lilbee's settings.`,
    NOTICE_AGENT_CONFIG_FAILED: (detail: string): string => `lilbee could not write the agent config: ${detail}`,
    NOTICE_CLAUDIAN_UPDATED: "Claudian's opencode provider is set up. Reload it to pick up the change.",
