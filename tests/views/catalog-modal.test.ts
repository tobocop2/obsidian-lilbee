...
function _findButtons(el: MockElement): MockElement[] {
...
describe("CatalogModal", () => {
    ...
    it("handles vision model task type in Use button", async () => {
        ...
        // Call handleUse directly to cover the vision branch
    ...
    it("handles embedding model task type in Use button", async () => {
        ...
        // Call handleUse directly to cover the embedding branch
        const variant = families[0].variants[0];
        const fakeBtn = { textContent: "", disabled: false };
        await (modal as any).handleUse(families[0], variant, fakeBtn as any);
        await vi.runAllTimersAsync();
    ...
    it("shows Use/Remove buttons for installed but not active variant", async () => {
        const families = [
            makeFamily({
                variants: [makeVariant({ name: "llava", hf_repo: "llava/v1.6", installed: true })],
            }),
        ];
        const plugin = makePlugin({ activeModel: "other-model", activeVisionModel: "other-vision" });
        plugin.api.catalog.mockResolvedValue(ok(makeCatalogResponse(families)));
        const app = new App();
        const modal = new CatalogModal(app as any, plugin as any);
        modal.open();
        ...
        expect(pullBtn).toBeNull();
    });

    it("shows Pull button for non-installed variant", async () => {
        const families = [
            makeFamily({
                variants: [makeVariant({ name: "llava", hf_repo: "llava/v1.6", installed: false })],
            }),
        ];
        const plugin = makePlugin();
        plugin.api.catalog.mockResolvedValue(ok(makeCatalogResponse(families)));
    ...
    it("renders multiple families from response", async () => {
        const families = [makeFamily({ family: "Qwen3" }), makeFamily({ family: "Llama3" })];
        const plugin = makePlugin();
        plugin.api.catalog.mockResolvedValue(ok(makeCatalogResponse(families)));
        const app = new App();
        const modal = new CatalogModal(app as any, plugin as any);
    ...
    it("renders recommended variant with star", async () => {
        const families = [
            makeFamily({
                family: "Test",
                recommended: "4B",
        ...
        const plugin = makePlugin({ activeModel: "test/4B" });
        plugin.api.catalog.mockResolvedValue(ok(makeCatalogResponse(families)));
        const app = new App();
        const modal = new CatalogModal(app as any, plugin as any);
        modal.open();
        ...
        const el = modal.contentEl as unknown as MockElement;
        const nameEl = el.find("lilbee-catalog-variant-name");
        expect(nameEl?.textContent).toContain("\u2605");
    });

    it("renders non-recommended variant without star", async () => {
        const families = [
            makeFamily({
                family: "Test",
                recommended: "other-variant",
                variants: [makeVariant({ name: "4B", hf_repo: "test/4B" })],
            }),
        ];
        const plugin = makePlugin();
    ...
    it("variant size and description are rendered", async () => {
        const families = [
            makeFamily({
                variants: [
                    makeVariant({ name: "4B", hf_repo: "test/4B", size_gb: 2.5, description: "Balanced model" }),
                ],
            }),
        ];
        const plugin = makePlugin({ activeModel: "other" });
