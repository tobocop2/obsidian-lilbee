...
- **Conversations that keep up.** Every chat [saves itself as you go](#conversations-you-can-come-back-to), titled from your first question; reopen any of them from the history button and pick up where you left off. When one outgrows the model's context, lilbee can [condense the oldest messages](#long-conversations-condensed-instead-of-dropped-optional) into a summary instead of dropping them, so the thread survives (optional, off by default).
