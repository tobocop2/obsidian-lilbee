...
<p align="center"><strong>The whole local AI stack for your vault. It downloads the models, runs them, and turns your notes into a search engine you can talk to.</strong></p>
...
This plugin runs **[lilbee](https://lilbee.sh/)** against your vault, inside Obsidian:

- Chat with click-to-source citations
- A web crawler that saves sites to your vault
- An auto-generated wiki
- A model catalog
...
- **Ask your vault in plain English.** [Type a question](#a-library-of-your-vault) and get an answer with citations that click straight back to the source line.
- **Verify in one click.** Every citation opens a [Source Preview](#verify-every-answer-at-the-source) scrolled to the exact spot. Surrounding paragraphs stay visible, and the cited lines are highlighted.
- **Save websites into your vault.** [Crawl a single page or a whole docs site](#offline-copies-of-websites-inside-your-vault) and the pages land in your vault as markdown notes. You search, chat with, and cite them offline, even after the site changes or goes down.
...
- **Your models, your machine.** [Browse a built-in model catalog](#pick-and-tune-your-models) straight from Hugging Face Hub, then pull one with a click and run it locally. No account needed.
- **Spreads models across your GPUs.** [Chat, embedding, reranking, and vision run as a fleet](#place-models-across-your-gpus). lilbee places them for you, and splits a model too big for one card across several. A GPU placement view lets you assign roles to cards by hand on a multi-GPU box.
- **Already on Ollama or LM Studio? Keep them.** lilbee manages models for you by default, but it [works with both](#already-running-ollama-or-lm-studio-keep-them). Their models appear in the same pickers, alongside lilbee's own, so you never have to switch model managers.
...
The lilbee README has a [full comparison table](https://github.com/tobocop2/lilbee#full-comparison-table). It adds the many-user serving story against vLLM, model sizes, and the interface list.
...
- a medical-textbook collection
- a field's research papers
- a company wiki
...
The save button in the chat toolbar writes the chat to a note in the `lilbee` folder of your vault. The note starts with the chat's title, model and dates, and each answer ends with its numbered sources. The save button in the history list does the same for any saved chat, without opening it. On desktop, the export button writes the same note to a file anywhere on your computer. You pick the file in the save dialog. The export button is in the chat toolbar and in the history list, and the **Export current chat to a file** command does the same for the open chat.
...
- **Prose and structured files.** PDFs, Word / Excel / PowerPoint, ebooks, CSV / JSON / YAML. Each chunk keeps its section context.
...
The plugin downloads and manages the [lilbee](https://lilbee.sh/) server for you, so there's nothing to install separately. The first launch fetches the right version for your platform and verifies it before starting.

Wait for the status bar to show `lilbee: ready`, then open the chat. The first-run reel near the top walks the whole flow, with a longer cut on the [tutorial page](https://obsidian.lilbee.sh/tutorial).

> **Hardware note:** the server runs on your CPU or GPU. A Mac with Apple Silicon (M1+) or a PC with an NVIDIA / AMD / Intel Arc GPU gives the best performance. 8 GB of RAM is the minimum, and 16 to 32 GB is recommended. See [lilbee's hardware requirements](https://github.com/tobocop2/lilbee#hardware-requirements) for the full table.
...
- **Search** (the default) answers _from your vault_. It retrieves the most relevant passages and replies with citations you click straight back to the source. Use it for grounded answers about your own notes, PDFs, and crawled pages.
...
The plugin runs [lilbee](https://lilbee.sh/) in the background. On first launch it downloads the right version for your platform, starts it, and shuts it down when you close Obsidian.
...
- **Managed or external server.** The plugin talks to a local lilbee server over `127.0.0.1`. In **managed mode** (the default) Obsidian runs the server for you, so you get going without installing or managing the executable yourself. It **prompts before downloading** the server, starts it when Obsidian launches, and stops it when you close Obsidian. In **external mode** you run the server yourself and point the plugin at it, keeping it alive however suits your setup ([as a service or daemon](https://github.com/tobocop2/lilbee#running-as-a-service-optional)). In that mode the plugin downloads no executable.
...
- **Vault access.** The plugin reads your vault to index it. If you enable the wiki, it writes generated pages into a vault folder you choose (default `lilbee/`). The server install and model cache live in Obsidian's app data folder, outside your vault.
...
See the **[Usage Guide](docs/usage.md)** for the full reference: every command, every setting, the chat toolbar, supported formats, troubleshooting, and advanced configuration. For the underlying engine, see [lilbee](https://lilbee.sh/): what it indexes, how retrieval works, model formats, and hardware requirements.
...
Something broken? See [TROUBLESHOOTING.md](TROUBLESHOOTING.md). Use **Settings → lilbee → Export diagnostics** to attach logs to a bug report.

lilbee is built and maintained by one person. If it's useful to you, you can chip in via [PayPal](https://paypal.me/lilbeedotsh). Bug reports and pull requests help just as much.
